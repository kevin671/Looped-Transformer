...
class WeightTyingBlock(nn.Module):

    def __init__(self, config, causal_self_attn, mlp):
        super().__init__()
        self.ln_1 = LayerNorm(config.n_embd, bias=config.bias)
        self.attn = causal_self_attn
...
class WeightTyingGPT(nn.Module):
    def __init__(self, config):
        ...
        if config.share_attn:
            causal_self_attn = CausalSelfAttention(config)
        ...
        for _ in range(self.config.n_layer):
            _causal_self_attn = (
                causal_self_attn if config.share_attn else CausalSelfAttention(config)
            )
            _mlp = mlp if config.share_mlp else MLP(config)
            block = WeightTyingBlock(config, _causal_self_attn, _mlp)
...
if __name__ == "__main__":
    ...
    config = WeightTyingGPTConfig(
        # share_attn=False,
        # share_mlp=False,
    )
    model = WeightTyingGPT(config)
