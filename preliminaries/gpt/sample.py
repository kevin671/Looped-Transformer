...
# -----------------------------------------------------------------------------
init_from = (
    "looped_gpt"  # either 'resume' (from an out_dir) or a gpt2 variant (e.g. 'gpt2-xl')
)
out_dir = (
    "out/looped_1_layer_200_loop_wikitext-103"  # ignored if init_from is not 'resume'
)
...
if init_from == "gpt":
    ...
    ckpt_path = os.path.join(out_dir, "ckpt.pt")
    checkpoint = torch.load(ckpt_path, map_location=device)
    gptconf = LoopedGPTConfig(**checkpoint["model_args"])
    ...
    ckpt_path = os.path.join(out_dir, "ckpt.pt")
    checkpoint = torch.load(ckpt_path, map_location=device)
    gptconf = DynamicLoopedGPTConfig(**checkpoint["model_args"])
