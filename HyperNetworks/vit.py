...
class Attention(nn.Module):
    ...
    def forward(self, x, qkv_weight):
        ...
        qkv = F.linear(x, qkv_weight).chunk(3, dim=-1)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.0):
        ...
        # self.to_qkv_layers = nn.ModuleList([])
        _qkv_weight = [
            torch.randn(dim_head * heads * 3, dim) for _ in range(time_embed_dim)
        ]
        for i in range(time_embed_dim):
            nn.init.kaiming_normal_(_qkv_weight[i])
        _qkv_weight = [_qkv_weight.flatten() for _qkv_weight in _qkv_weight]
        self.qkv_weight = nn.Parameter(torch.stack(_qkv_weight))
        # print(self.qkv_weight.shape)  # torch.Size([16, 294912])
