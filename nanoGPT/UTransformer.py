...
from model import MLP, CausalSelfAttention, LayerNorm


@dataclass
class UniversalGPTConfig:
    block_size: int = 1024
    vocab_size: int = (
...
class UniversalGPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert config.n_layer == 1  # only a single layer is supported for now
...
if __name__ == "__main__":
    ...
    config = UniversalGPTConfig()
    model = UniversalGPT(config).to(device)
