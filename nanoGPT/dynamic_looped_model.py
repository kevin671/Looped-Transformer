...
class DynamicLoopedGPT(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        config = DynamicLoopedGPTConfig(**config_args)
...
if __name__ == "__main__":
    ...
    model = DynamicLoopedGPT(config)
    ckpt = torch.load(
        "/work/gg45/g45004/Looped-Transformer/nanoGPT/out/dynamic_looped_100_wikitext-103/ckpt.pt",
        map_location="cpu",
    )
    ...
    input = torch.randint(0, 50257, (4, 12))
    output = model(input)
# %%
