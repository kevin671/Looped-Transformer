...
if __name__ == "__main__":
    ...
    input_d = 20
    d_model = 128
    qkv_proj = MergedLinear(input_d, 3 * d_model, r=8, enable_lora=[True, False, True])

    x = torch.randn(batch_size, seq_len, input_d)

    q_proj = Linear(input_d, d_model, r=8)
    y = q_proj(x)
    print(y.shape)  # torch.Size([10, 20, 128]

    y = qkv_proj(x)
    print(y.shape)  # torch.Size([10, 384])
# %%
