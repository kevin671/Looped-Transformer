...
def make_model_with_empty_targets(
    ...
    def new_model(x: chex.Array) -> chex.Array:
        ...
        computation_tape = jnn.one_hot(computation_tape, num_classes=final_input_size)
        ...
        if "input_length" in inspect.getfullargspec(model).args:
            output = model(
                final_input, input_length=input_length
...
def make_model_with_targets_as_input(
    ...
    def new_model(x: chex.Array, y: chex.Array) -> chex.Array:
        ...
        final_input = jnp.concatenate([x, computation_tape, output_token, y], axis=1)
...
def update_tree_with_new_containers(tree: Any, update_dict: dict[str, Any]) -> None:
